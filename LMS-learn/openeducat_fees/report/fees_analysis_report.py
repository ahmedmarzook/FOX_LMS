...
class ReportFeesAnalysis(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        data = data or {}
        fees_filter = data.get('fees_filter')

        if fees_filter == 'student' and data.get('student'):
            student_ids = self.env['op.student'].browse([data['student']])
        elif fees_filter == 'course' and data.get('course'):
            student_ids = self.env['op.student'].search(
                [('course_detail_ids.course_id', '=', data['course'])])
        else:
            student_ids = self.env['op.student'].browse(docids)
