...
class OpJobApplicant(models.Model):
    ...
    def write(self, vals):
        ...
        values['date_last_stage_update'] = fields.Datetime.now()
        values.update(
            self._onchange_stage_id_internal(values.get('stage_id'))['value']
        )
        values.setdefault('kanban_state', 'normal')
        result = True
        for applicant in self:
            record_values = dict(values)
            record_values['last_stage_id'] = applicant.stage_id.id
            result = super(OpJobApplicant, applicant).write(record_values) and result
        return result
