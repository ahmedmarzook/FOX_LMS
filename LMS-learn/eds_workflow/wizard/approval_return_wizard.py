...
class ApprovalReturnWizard(models.TransientModel):
    ...
    def action_return(self):
        ...
        record = self.env[self._context.get("active_model")].browse(self.record_id)
        record_id = (
            self.env["boarding.order.form"].sudo().search([("id", "=", record.id)])
        )
        record_id.description = self.reason
