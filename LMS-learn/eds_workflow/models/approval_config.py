...
class ApprovalConfig(models.Model):
    ...
    def _get_sequence(self):
        ...
        self._cr.execute(
            "select max(sequence) from %s where model_id =%s" % (self._table, model_id)
        )
    ...
    template_ids = fields.Many2many(
        ...
        relation="approval_config_mail_template_rel",
        domain="[('model_id','=', model_id)]",
    )
    reject_template_ids = fields.Many2many(
        "mail.template",
        string="Reject Mail Templates",
        relation="approval_config_reject_template_ids_rel",
        domain="[('model_id','=', model_id)]",
    )

    auto_subscribe_users = fields.Boolean(
        "Add followers", help="Add Approval Users to the record followers"
    )

    auto_subscribe_users_enabled = fields.Boolean(
        compute="_calc_auto_subscribe_users_enabled"
    )
    ...
    schedule_activity_enabled = fields.Boolean(
        "Hours", compute="_calc_schedule_activity_enabled"
    )

    post_approval_msg = fields.Boolean("Post Approval Message")
    post_reject_msg = fields.Boolean("Post Reject Message")

    before_script = fields.Text(
        "Before Script", help="Script to Execute on Enter this State"
    )
    after_script = fields.Text(
        "After Script", help="Script to Execute after Exit this State"
    )
    ...
    last_state_update_id = fields.Many2one(
        "ir.model.fields", compute="_calc_last_state_update_id"
    )
    ...
    is_sudo_approve = fields.Boolean(
        string="Approve as super user",
        help="Enable this option when a user can only approve a record but cannot edit, user access rights: only read.",
    )

    cancel_type = fields.Selection(
        [("auto_cancel", "Cancel Automatic"), ("workflow", "Cancel Workflow")],
        string="Cancel Type",
    )
    ...
    default_reject_mail_template_body = fields.Char(
        compute="_calc_default_mail_template"
    )
    ...
    def _calc_default_mail_template(self):
        for record in self:
            record.default_mail_template_body = self.env.ref(
                "eds_workflow.approval_notification_default"
            ).arch
            record.default_reject_mail_template_body = self.env.ref(
                "eds_workflow.reject_notification_default"
            ).arch
    ...
    def _calc_last_state_update_id(self):
        for record in self:
            record.last_state_update_id = self.env["ir.model.fields"]._get(
                record.model, "last_state_update"
            )
    ...
    def _calc_schedule_activity_enabled(self):
        for record in self:
            model = self.env[record.model_id.model]
            record.schedule_activity_enabled = model._isinstance(
                "mail.activity.mixin"
            ) and model._isinstance("mail.thread")
    ...
    def _get_next(self, record):
        if not self:
            ...
            return self.search(
                [("active", "=", True), ("model_id", "=", model_id)], limit=1
            )._next(record)
    ...
    def _next(self, record, force=False):
        ...
        if not force:
            try:
                ...
                _logger.error(
                    "Error evaluating workflow condition %s" % [record, self.state]
                )
    ...
    def _update_approval_activity(self):
        for group in self.read_group(
            [("schedule_activity", "=", True)], ["model_id"], ["model_id"]
        ):
    ...
    def _run_update_approval_activity(self):
        try:
            with self.env.cr.savepoint():
                self.env.ref("eds_workflow.cron_update_approval_activity").write(
