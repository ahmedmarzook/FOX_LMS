...
class ApprovalForward(models.Model):
    ...
    status_id = fields.Many2one("approval.config", required=True, ondelete="cascade")
    ...
    _sql_constraints = [
        (
            "duplicate_check",
            "EXCLUDE USING btree (model_id WITH =, record_id WITH =, status_id WITH =) WHERE (active)",
            "Duplicate Forward Record!",
        )
    ]
