...
class TeamsMeet(models.TransientModel):
    ...
    def create_meeting(self):
        timez = str(self.env.user.tz)
        res_config = self.env['ir.config_parameter'].sudo()
        self.env['res.config.settings'].sudo().create({}).do_refresh_token()
        for res in self: # noqa
            webhook_url_key = res_config.search([('key', '=', 'openeducat_teams.webhook_url')])
            access_token_key = res_config.search(
                [('key', '=', 'openeducat_teams.access_token')])
            send_card_key = res_config.search([['key', '=', 'openeducat_teams.send_card_globally']])
            course_send_card_key = res_config.search(
                [['key', '=', 'openeducat_teams.send_card_course']])
        ...
        try:
            ...
            user_response_data.raise_for_status()
            json_data = user_response_data.json()
        except requests.RequestException as error:
            raise UserError(
                _("Microsoft Teams request failed: %s") % error
            ) from error
