...
class MeetingPortal(CustomerPortal):
    ...
    @http.route('/get-meeting/data', type='jsonrpc', auth='user', website=True)
        ...
        for meeting in meeting_id:
            data.append({
                ...
                'start': fields.Datetime.context_timestamp(
                    request.env.user, meeting.start).isoformat() if meeting.start else False,
                'end': fields.Datetime.context_timestamp(
                    request.env.user, meeting.stop).isoformat() if meeting.stop else False,
                'meeting_url': getattr(meeting, 'meeting_url', False) or
                               getattr(meeting, 'videocall_location', False),
