...
class OpDigitalLibraryController(CustomerPortal):
    ...
    def add_to_reading_list(self, material_id):
        material_model = request.env['op.digital.library.material'].sudo().browse(int(material_id)).exists()
    ...
    def add_review_material_list(self, **post):
        ...
        if post:
            post.update({
                ...
                'rating': float(post.get('rating') or 0),
            })
    ...
    def digital_library_category_detail(self, material_id=0):
        material = request.env['op.digital.library.material'].sudo().browse(int(material_id)).exists()
