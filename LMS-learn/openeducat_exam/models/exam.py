...
class OpExam(models.Model):
    ...
    def _check_marks(self):
        for exam in self:
            if exam.total_marks <= 0.0 or exam.min_marks <= 0.0:
                raise ValidationError(_('Enter proper marks!'))
            if exam.min_marks > exam.total_marks:
    ...
    def _check_date_time(self):
        for exam in self:
            ...
            if not exam.session_id or not exam.session_id.start_date or not exam.session_id.end_date:
                continue
            session_start = datetime.datetime.combine(
                fields.Date.from_string(exam.session_id.start_date),
                datetime.time.min)
            session_end = datetime.datetime.combine(
                fields.Date.from_string(exam.session_id.end_date),
                datetime.time.max)
            if start_time < session_start or start_time > session_end or \
