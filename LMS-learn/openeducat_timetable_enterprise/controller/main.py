...
class TimeTablePortal(CustomerPortal):
    ...
    def _parent_prepare_portal_layout_values(self, student_id=None):
        ...
        student = request.env['op.student'].sudo().browse(student_id).exists()
        timetable_count = 0
    ...
    @http.route('/get-timetable/data', type='jsonrpc', auth='user', website=True)
    def get_timetable_data_portal(self, stud_id=None, current_timezone=None):
        ...
        if stud_id:
            ...
            student = request.env['op.student'].sudo().browse(int(stud_id)).exists()
        else:
            student = request.env['op.student'].sudo().search(
        ...
        for session in session_model:
            all_lession = ' '.join(
                lesson.lesson_topic or '' for lesson in session.lesson_ids
            ).strip()
            data.append({
