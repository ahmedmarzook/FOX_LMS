...
class OnboardingController(http.Controller):
    ...
    @http.route('/openeducat_exam_enterprise/'
                'openeducat_exam_onboarding_panel',
                auth='user', type='jsonrpc')
    def openeducat_exam_onboarding_panel(self):
        ...
        if not request.env.user._is_admin() or \
                company.openeducat_exam_onboard_panel == 'closed':
            return {}
        ...
        return {
            'html':
                request.env['ir.qweb']._render(
                    ...
                    'openeducat_exam_onboarding_panel',
                    {'company': company,
                     'state': company.update_exam_onboarding_state()})
        }
