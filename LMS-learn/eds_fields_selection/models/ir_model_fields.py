...
class IrModelFields(models.Model):
    ...
    def _calc_selection_count(self):
        for record in self:
            record.selection_count = self.env[
                "ir.model.fields.selection.custom"
            ].search_count([("field_id", "=", record.id)])

    def _calc_default_count(self):
        for record in self:
            record.default_count = self.env["ir.default"].search_count(
                [("field_id", "=", record.id)]
            )

    def action_selections(self):
        assert self.ttype in ["selection", "reference"]
        if not self.env["ir.model.fields.selection.custom"].search(
            [("field_id", "=", self.id)], limit=1
        ):
            env = self.env
            trl = dict()
            for lang_id in self.env["res.lang"].search(
