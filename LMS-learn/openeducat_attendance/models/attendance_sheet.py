...
class OpAttendanceSheet(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sheet = self.env['ir.sequence'].next_by_code('op.attendance.sheet') or ''
            register = self.env['op.attendance.register'].browse(
                vals.get('register_id')
            )
            vals['name'] = (register.code or '') + sheet
