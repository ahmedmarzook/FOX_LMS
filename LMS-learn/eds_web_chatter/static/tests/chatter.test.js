...
import "@eds_web_chatter/chatter/chatter";

test.tags("eds_web_chatter");
test("notifications toggle updates localStorage and state", async () => {
    browser.localStorage.removeItem("eds_web_chatter.notifications");
    browser.localStorage.setItem(
        "eds_web_chatter.notifications", JSON.stringify(false)
    );
    const chatter = {
        state: {
