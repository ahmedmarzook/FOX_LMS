...
class OpActivityAnnouncement(models.Model):
    ...
    job_post_id = fields.Many2many('op.job.post',
                                   ...
                                   tracking=True)
    skill_id = fields.Many2many('op.student.skill.name', string='Required Skills')
    ...
    follower_email_to = fields.Char(
        string="Follower Emails",
        compute="_compute_follower_email_to",
    )

    def _compute_follower_email_to(self):
        for record in self:
            emails = record.message_follower_ids.mapped("partner_id.email")
            record.follower_email_to = ",".join(
