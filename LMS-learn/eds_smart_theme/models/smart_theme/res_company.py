...
class Company(models.Model):
    ...
    tab_name = fields.Char(string="Backend Tab Name", default="Smart", readonly=False)
    backend_theme_level = fields.Selection(
        [("user_level", "User Level"), ("global_level", "Global Level")],
        default="user_level",
        required=True,
    ...
    login_page_background_img = fields.Binary(
        "Login Background Image", readonly=False, store=True
    )
    login_page_background_color = fields.Char(
        "Login Background Color", default="#f2f6ff", readonly=False
    )
    login_page_text_color = fields.Char(
        "Login Text Color", default="#777777", readonly=False
    )
    ...
    def get_login_page_data(self):
        ...
        if admin_config:
            ...
            config_vals = (
                request.env["backend.config"].sudo().search([], order="id asc", limit=1)
            )
    ...
    backend_menubar_logo = fields.Binary(string="Menubar Logo", readonly=False)
    ...
    app_name_pwa = fields.Char("App Name", readonly=False, default="Smart")
    short_name_pwa = fields.Char("Short Name", readonly=False, default="Smart")
    description_pwa = fields.Char("App Description", readonly=False, default="Smart")
    ...
    background_color_pwa = fields.Char(
        "Background Color", readonly=False, default="#3a2e66"
    )
    theme_color_pwa = fields.Char("Theme Color", readonly=False, default="#3a2e66")
    pwa_shortcuts_ids = fields.Many2many("pwa.shortcuts", string="PWA Shortcuts")

    spiffy_toobar_color = fields.Char(
        "Toolbar Color", readonly=False, default="#3a2e66"
    )
    ...
    prevent_auto_save_warning = fields.Char(
        "Auto Save Warning",
        translate=True,
        default="Autosave is disabled, Click on save button.",
        readonly=False,
    )
