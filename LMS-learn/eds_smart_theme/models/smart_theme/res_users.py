...
class User(models.Model):
    ...
    backend_theme_config = fields.Many2one(
        "backend.config", string="Backend Config", copy=False
    )
    ...
    def _smart_show_edit_mode(self):
        """Whether the current user may open the theme configurator."""
        self.ensure_one()
        if self.env.company.backend_theme_level == "user_level":
            return True
        admin_users = (
            self.env["res.users"]
            .sudo()
        ...
        return self.id in admin_users.ids
