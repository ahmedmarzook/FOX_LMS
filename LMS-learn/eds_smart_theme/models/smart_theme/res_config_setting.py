...
class ResConfig(models.TransientModel):
    ...
    tab_name = fields.Char(
        related="company_id.tab_name", string="Backend Tab Name", readonly=False
    )
    backend_theme_level = fields.Selection(
        related="company_id.backend_theme_level",
    ...
    login_page_style = fields.Selection(
        related="company_id.login_page_style",
        string="Login Styles",
        required=True,
        readonly=False,
    )
    ...
    login_page_background_img = fields.Binary(
        related="company_id.login_page_background_img",
        string="Login Background Image",
        readonly=False,
    )

    login_page_background_color = fields.Char(
        related="company_id.login_page_background_color",
        string="Login Background Color",
        readonly=False,
    )

    login_page_text_color = fields.Char(
        related="company_id.login_page_text_color",
        string="Login Text Color",
        readonly=False,
    )

    show_bg_image = fields.Boolean(
        related="company_id.show_bg_image",
        string="Add Login Background Image",
        readonly=False,
    )
    spiffy_favicon = fields.Binary(
        related="company_id.spiffy_favicon",
        string="Backend Tab Favicon",
        readonly=False,
    )
    ...
    backend_menubar_logo_icon = fields.Binary(
        related="company_id.backend_menubar_logo_icon",
        string="Menubar Logo Icon",
        readonly=False,
    )
    ...
    spiffy_toobar_color = fields.Char(
        "Toolbar Color", related="company_id.spiffy_toobar_color", readonly=False
    )

    prevent_auto_save = fields.Boolean(
        related="company_id.prevent_auto_save",
        string="Prevent Auto Save ?",
        readonly=False,
    )

    prevent_auto_save_warning = fields.Char(
        "Auto Save Warning",
        related="company_id.prevent_auto_save_warning",
        readonly=False,
    )

    firebase_server_key = fields.Char(
        string="Firebase key", related="company_id.firebase_server_key", readonly=False
    )
    firebase_key_file = fields.Binary(
        "Firebase Key File", readonly=False, related="company_id.firebase_key_file"
    )

    def set_values(self):
        res = super(ResConfig, self).set_values()
        config_parameters = self.env["ir.config_parameter"]
        config_parameters.set_param("firebase_server_key", self.firebase_server_key)
        return res

    @api.model
    def get_values(self):
        res = super(ResConfig, self).get_values()
        firebase_server_key = self.env["ir.config_parameter"].get_param(
            "firebase_server_key"
        )
        firebase_key_file = self.env["ir.config_parameter"].get_param(
            "firebase_key_file"
        )
        res.update(
            firebase_server_key=firebase_server_key, firebase_key_file=firebase_key_file
        )
        return res
