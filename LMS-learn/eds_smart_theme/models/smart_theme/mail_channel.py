...
class MailThread(models.AbstractModel):
    ...
    def _notify_record_firebase(self, message, rdata, msg_vals=False, **kwargs):
        ...
        no_inbox_pids = [
            r["id"] for r in rdata if r["active"] and r["notif"] != "inbox"
        ]
        if not notif_pids:
            return
        ...
        if msg_type in ("comment", "wa_message"):
            ...
            for id in pids:
                ...
                device_ids = (
                    user_obj.mail_firebase_tokens.mapped("token") if user_obj else False
                )
    ...
    def _mail_channel_firebase_notifications(self, message, device_ids):
        ...
        if not key:
            raise UserError(
                _("Firebase key file for Smart app is missing from the configuration.")
            )
        ...
        try:
            ...
            menu_item_obj = (
                self.env["push.notification.menu"]
                .sudo()
                .search([("model_name", "=", message["model"])], limit=1)
            )
            title_msg = ""
            ...
            if message["model"] == "discuss.channel":
                ...
                whatsapp_chatroom_obj = self.env["whatsapp.chatroom"].search(
                    [("id", "=", message["res_id"])]
                )
                active_waba_id = (
                    whatsapp_chatroom_obj.wa_business_acc_id.id
                    if whatsapp_chatroom_obj
                    else False
                )
                active_action = self.env.ref(
                    "whatsapp_integration_bs_chatroom.bs_whatsapp_chatroom"
                )
...
class Channel(models.Model):
    ...
    def _notify_record_firebase(self, message, rdata, msg_vals=False, **kwargs):
        ...
        channel_channels = self.filtered(
            lambda channel: channel.channel_type == "channel"
        )
        if chat_channels:
            ...
            channel_rdata = rdata
        return super(Channel, self)._notify_record_firebase(
            message, channel_rdata, msg_vals=msg_vals, **kwargs
        )
