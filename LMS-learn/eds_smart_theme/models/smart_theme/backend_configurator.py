...
class BackendConfig(models.Model):
    ...
    def _default_app_drawer_bg_image(self):
        image_path = file_path(
            "eds_smart_theme/static/src/smart/images/app-drawer-bg-image.png"
        )
    ...
    def _default_menu_bg_image(self):
        image_path = file_path(
            "eds_smart_theme/static/src/smart/images/header_vertical_mini.svg"
        )
        with tools.file_open(image_path, "rb") as f:
            return base64.b64encode(f.read())
    ...
    drawer_color_pallet = fields.Selection(
        ...
        default="drawer_pallet_9",
        string="Drawer Color Pallets",
    )

    appdrawer_custom_bg_color = fields.Char(
        string="App Drawer Custom Background Color", default="#3a2e66"
    )
    appdrawer_custom_text_color = fields.Char(
        string="App Drawer Custom Text Color", default="#ffffff"
    )

    header_vertical_mini_text_color = fields.Char(
        string="App Header Custom Text Color", default="#2d678b"
    )
    header_vertical_mini_bg_color = fields.Char(
        string="App Header Custom BG Color", default="#f9fdff"
    )

    menu_shape_bg_color = fields.Char(string="Menu Shape BG Color", default="#000000")
    menu_shape_bg_color_opacity = fields.Float(
        string="Menu Shape BG Color Opacity", default="1"
    )

    light_primary_bg_color = fields.Char(
        string="Primary Background Color for light", default="#3a2e66"
    )
    light_primary_text_color = fields.Char(
        string="Primary Text Color for light", default="#ffffff"
    )

    apply_light_bg_img = fields.Boolean(string="Apply light bg image")
    light_bg_image = fields.Binary(
        string="Background Image For light",
        default=_default_app_drawer_bg_image,
        readonly=False,
    )

    menu_bg_image = fields.Binary(
        string="Background Image For menu",
        default=_default_menu_bg_image,
        readonly=False,
    )

    dark_primary_bg_color = fields.Char(
        string="Primary Background Color for dark", default="#3a2e66"
    )
    dark_primary_text_color = fields.Char(
        string="Primary Text Color for dark", default="#ffffff"
    )

    dark_secondry_bg_color = fields.Char(
        string="Secondry Background Color for dark", default="#242424"
    )
    dark_secondry_text_color = fields.Char(
        string="Secondry Text Color for dark", default="#ffffff"
    )

    dark_body_bg_color = fields.Char(
        string="Body Background Color for dark", default="#1d1d1d"
    )
    dark_body_text_color = fields.Char(
        string="Body Text Color for dark", default="#ffffff"
    )
    ...
    checkbox = fields.Selection(
        ...
        default="checkbox_style_4",
        string="Checkbox Styles",
    )
    ...
    popup = fields.Selection(
        ...
        default="popup_style_2",
        string="popup Styles",
    )

    chatter_position = fields.Selection(
        [("chatter_right", "Chatter Right"), ("chatter_bottom", "Chatter Bottom")],
    ...
    top_menu_position = fields.Selection(
        ...
        default="top_menu_vertical",
        string="Top Menu Position",
    )
    ...
    loader_style = fields.Selection(
        ...
        default="loader_style_10",
        string="Loader Styles",
    )
    ...
    list_view_density = fields.Selection(
        ...
        default="list_comfortable",
        string="List View Density",
    )
    ...
    input_style = fields.Selection(
        ...
        default="input_bottom_border",
        string="Form View Mode",
    )

    list_view_sticky_header = fields.Boolean(string="List view Sticky Header")

    @api.model
    def _migrate_default_theme_color(self):
        param = self.env["ir.config_parameter"].sudo()
        new = "#3a2e66"
        if param.get_param("eds_smart_theme.default_color_migrated") == new:
            return
        old_colors = ["#0097a7", "#2b254b"]
        self.search(
            [
                "|",
                ("color_pallet", "=", "pallet_9"),
                ("light_primary_bg_color", "in", old_colors),
            ]
        ).write(
            {
                "light_primary_bg_color": new,
                "dark_primary_bg_color": new,
                "appdrawer_custom_bg_color": new,
            }
        )
        self.env["res.company"].sudo().search(
            [
                "|",
                ("theme_color_pwa", "in", old_colors),
                ("spiffy_toobar_color", "in", old_colors),
            ]
        ).write(
            {
                "background_color_pwa": new,
                "theme_color_pwa": new,
                "spiffy_toobar_color": new,
            }
        )
        param.set_param("eds_smart_theme.default_color_migrated", new)

    @api.model
    def _migrate_default_font_family(self):
        param = self.env["ir.config_parameter"].sudo()
        if param.get_param("eds_smart_theme.default_font_migrated"):
            return
        self.search([("font_family", "in", [False, "rubik"])]).write(
            {
                "font_family": "vazirmatn",
            }
        )
        param.set_param("eds_smart_theme.default_font_migrated", "1")
