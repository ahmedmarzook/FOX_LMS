...
class PwaMain(http.Controller):
    ...
    def service_worker(self):
        ...
        content = qweb._render(
            "eds_smart_theme.service_worker",
            {
    ...
    def pwa_offline(self, **kw):
        return request.render(
            "eds_smart_theme.pwa_offline_page",
        )

    @http.route(
        "/eds_smart_theme/<int:company_id>/manifest.json", type="http", auth="public"
    )
    def manifest(self, company_id=None):
        company = (
            request.env["res.company"].search([("id", "=", company_id)])
            if company_id
        ...
        image_192_pwa = "/web/image/res.company/%s/image_192_pwa/192x192" % (company.id)
        image_512_pwa = "/web/image/res.company/%s/image_512_pwa/512x512" % (company.id)
        pwa_content = {
            "name": app_name_pwa,
            "short_name": short_name_pwa,
            "icons": [
                {"sizes": "192x192", "src": image_192_pwa, "type": "image/png"},
                {"sizes": "512x512", "src": image_512_pwa, "type": "image/png"},
            ],
        ...
        if company.pwa_shortcuts_ids:
            for pwashorts in company.pwa_shortcuts_ids:
                dict = {
                    ...
                    "icons": [
                        {
                            "src": "/web/image/res.company/%s/image_192_shortcut"
                            % (company.id),
                            "sizes": "192x192",
                        }
                    ],
                ...
                pwa_content.update({"shortcuts": pwashortlist})
        return request.make_response(
            data=json.dumps(pwa_content), headers=[("Content-Type", "application/json")]
        )
