...
class BackendConfigration(http.Controller):
    ...
    def get_selected_pallet(self, **kw):
        ...
        if current_user.backend_theme_config:
            ...
            backend_config_record = (
                request.env["backend.config"].sudo().create(config_vals)
            )
    ...
    def selected_pallet_data(self, **kw):
        ...
        admin_users = (
            ...
            .search(
                [
                    ("all_group_ids", "in", request.env.ref("base.group_system").id),
        ...
        if company.backend_theme_level == "user_level":
            if user.backend_theme_config:
                ...
                config_vals = (
                    request.env["backend.config"]
                    .sudo()
                    .search([], order="id asc", limit=1)
                )
        else:
            if admin_config:
                config_vals = admin_config
        ...
        values = {}
        separator_selection_dict = dict(config_vals._fields["separator"].selection)
        tab_selection_dict = dict(config_vals._fields["tab"].selection)
        checkbox_selection_dict = dict(config_vals._fields["checkbox"].selection)
        ...
        response = request.render("eds_smart_theme.template_backend_config_data", values)
    ...
    def get_record_data(self, **kw):
        ...
        show_edit_mode = user._smart_show_edit_mode()
    ...
    def get_favorite_apps(self, **kw):
        ...
        if user_id.app_ids:
            for app in user_id.app_ids:
                irmenu = (
                    request.env["ir.ui.menu"].sudo().search([("id", "=", app.app_id)])
                )
                if irmenu:
                    app_dict = {
                        "name": app.name,
    ...
    def remove_favorite_apps(self, **kw):
        ...
        for line in user_id.app_ids:
            if line.app_id == str(kw.get("app_id")):
                user_id.sudo().write({"app_ids": [(3, line.id)]})
        return True

    @http.route(["/get/active/menu"], type="json", auth="public")
    def get_active_menu_data(self, **kw):
        menu_items = []
        menu_records = request.env["ir.ui.menu"].search([("parent_id", "=", False)])
        for menu in menu_records:
            menu_items.append({"menu_name": menu.complete_name, "menu_id": menu.id})
        return menu_items
                ...
                if record.parent_path:
                    ...
                    if parent_record_id == menuOption:
                        if not record.child_id:
                            menu_items.append(
                                {"name": record.complete_name, "menu_id": record.id}
                            )
        else:
            for record in menu_records:
                if not record.child_id:
    ...
    def get_active_lang(self, **kw):
        lang_records = request.env["res.lang"].sudo().search([("active", "=", "True")])
        lang_list = []
    ...
    def text_color_label_color(self, **kw):
        ...
        if "options" in kw:
            if "file_generator" and "options" in kw["options"]:
                ...
                allowed_company_ids = [
                    company_data["id"]
                    for company_data in options.get("multi_company", [])
                ]
                ...
                if type_ in ["qweb-pdf", "qweb-text"]:
                    ...
                    report_data = (
                        ...
                        ._render_qweb_pdf(reportname, docids, data=data)[0]
                    )

                    report_obj = request.env["ir.actions.report"]
                    filereport = (
                        report_obj.with_context(context)
                        .sudo()
                        .search([("report_name", "=", reportname)], limit=1)
                    )
                ...
                if not import_compat and groupby:
                    ...
                    with ExportXlsxWriter(
                        columns_headers, len(export_data)
                    ) as xlsx_writer:
                ...
                for row in jdata["rows"]:
                    worksheet.write(
                        y, x, row["indent"] * "     " + str(row["title"]), header_plain
                    )
    ...
    def mail_attachment_upload_from_app(
        ...
        if (
            ...
            and not thread.allow_public_upload
    ...
    def remove_multi_tab(self, **kw):
        multi_tab = (
            request.env["biz.multi.tab"]
            .sudo()
            .search([("id", "=", kw.get("multi_tab_id"))])
        )
        multi_tab.unlink()
        user = request.env.user
    ...
    def update_tabaction(self, **kw):
        ...
        multi_tab = request.env["biz.multi.tab"].sudo().search([("id", "=", tabId)])
        if multi_tab:
    ...
    def add_bookmark_link(self, **kw):
        user = request.env.user
        bookmark_ids = user.bookmark_ids.filtered(lambda b: b.name == kw.get("name"))
        if not bookmark_ids:
            user.sudo().write(
                {
                    "bookmark_ids": [
    ...
    def update_bookmark_link(self, **kw):
        bookmark = (
            request.env["bookmark.link"]
            .sudo()
            .search([("id", "=", kw.get("bookmark_id"))])
        )
    ...
    def get_irmenu_icondata(self, **kw):
        ...
        spiffy_app_group = (
            request.env["spiffy.app.group"]
            .sudo()
            .search_read([], fields=["id", "name", "group_menu_icon", "sequence"])
        )
        spiffy_app_group = sorted(spiffy_app_group, key=lambda group: group["sequence"])

        irmenu = (
            request.env["ir.ui.menu"].sudo().search([("id", "in", kw.get("menu_ids"))])
        )
        ...
        for group in groups:
            group_data = {"id": group.id, "name": group.name, "menus": []}
                ...
                group_data["menus"].append(
                    {
                        ...
                        "icon_img": (
                            menu.icon_img.decode("utf-8") if menu.icon_img else ""
                        ),
                    }
                )
        ...
        for menu in irmenu:
                    ...
                    spiffy_app_group_data = (
                        ...
                        .search_read(
                            ...
                            fields=[
                                ...
                                "group_menu_list_ids",
                                "use_group_icon",
                                "group_icon_class_name",
                            ],
                        )
                    )
                    if spiffy_app_group_data:
                        app_menu_dict.setdefault("spiffy_app_group", []).append(
                            spiffy_app_group_data[0]
                        )
            ...
            [
                item.update({"app_menu_list": app_menu_list})
                for item in app_menu_dict[menu.id]
                if "app_menu_list" in item
            ]

        if "spiffy_app_group" in app_menu_dict:
            app_menu_dict["spiffy_app_group"] = sorted(
                app_menu_dict["spiffy_app_group"], key=lambda group: group["sequence"]
            )
    ...
    def show_user_todo_list(self, **kw):
        ...
        response = request.render("eds_smart_theme.to_do_list_template", values)
    ...
    def create_todo(self, **kw):
        ...
        if user_id and (note_title or note_description):
            ...
            note_content = request.env["ir.ui.view"]._render_template(
                "eds_smart_theme.to_do_list_content_template",
                {
                    "note": todo_record,
                    "today_date": today_date_with_offset,
                    "user_tz_offset_time": user_tz_offset_time,
    ...
    def ThemecolorParameterCheck(self, uid, **post):
        module_obj = (
            ...
            .sudo()
            .search([("name", "=", "eds_smart_theme"), ("state", "=", "installed")])
        )
        if module_obj:
            if post.get("color_data"):
                ...
                view_obj = (
                    ...
                    .sudo()
                    .search(["|", ("key", "=", color_data), ("key", "=", theme_color)])
                )
                if view_obj:
        ...
        if uid == "null" and request.session.get("pre_uid"):
            ...
            if "device_token" and "device_name" in post:
                ...
                if device_token and device_name:
                    user_obj = (
                        request.env["mail.firebase"]
                        .sudo()
                        .search(
                            [("user_id", "=", int(uid)), ("token", "=", device_token)]
                        )
                    )
                    if not user_obj:
...
class WebManifest(SpiffyWebManifest):
    def _icon_path(self):
        return "eds_smart_theme/static/src/smart/images/loader_2.gif"
            ...
            try:
                ...
                if "tool_color_id" in kwargs:
                    ...
                    if device_name and device_token:
                        user_obj = request.env["mail.firebase"].search(
                            [("user_id", "=", int(uid)), ("token", "=", device_token)]
                        )
                        if not user_obj:
                            request.env["mail.firebase"].create(
                                {
                                    "user_id": int(uid),
                                    "os": device_name,
                                    "token": device_token,
