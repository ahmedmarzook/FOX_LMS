...
class BackendConfigrationRec(http.Controller):
    ...
    def _get_user_specific_domain(self, user):
        """
        Computes a domain for global search based on the modules
        installed and the current user's access rights.
        """
        # 1. Get names of all installed modules
        installed_modules = (
            request.env["ir.module.module"].sudo().search([("state", "=", "installed")])
        )
        module_names = installed_modules.mapped("name")

        # 2. Find groups associated with these modules via XML data
        # This identifies which groups belong to the installed apps
        groups_in_modules = (
            request.env["ir.model.data"]
            .sudo()
            .search([("module", "in", module_names), ("model", "=", "res.groups")])
            .mapped("res_id")
        )

        # 3. Filter modules where the user actually has a membership in those groups
        # This confirms the user has 'rights' within the scope of installed apps
        user_group_ids = set(user.all_group_ids.ids)
        valid_module_names = []

        # We re-check the mapping to ensure we only include models from modules
        # where the user has at least one related group assigned
        for module in installed_modules:
            module_groups = (
                request.env["ir.model.data"]
                .sudo()
                .search([("module", "=", module.name), ("model", "=", "res.groups")])
                .mapped("res_id")
            )

            if any(gid in user_group_ids for gid in module_groups):
                valid_module_names.append(module.name)

        # 4. Get all technical model names linked to those modules
        # ir.model.modules is a non-stored compute field (not usable in a search
        # domain), so filter in Python instead.
        valid_module_names_set = set(valid_module_names)
        installed_models = [
            m.model
            for m in request.env["ir.model"].sudo().search([])
            if m.modules and set(m.modules.split(", ")) & valid_module_names_set
        ]

        # 5. Final Filtering: Check actual 'Read' access and Registry existence
        accessible_models = []
        for model_name in installed_models:
            # SAFETY CHECK: Ensure the model exists in the Python Registry
            # This prevents the KeyError: 'mask.annual.reward...'
            if model_name not in request.env:
                continue

            # Optimization: Exclude Transient/Wizard models from global search
            if request.env[model_name]._transient:
                continue

            try:
                # Check if the specific user has 'read' permission
                if (
                ...
                # If any unexpected error occurs during check, skip the model
                continue

        # 6. Create domain for the 'global.search' (or similar) model
        # Matches 'global_model_id' (a Many2one to ir.model) against our allowed list
    ...
    #         user_has_rights = any(group_id in user.all_group_ids.ids for group_id in groups)
    #
    ...
    #             if request.env[model].sudo(False).check_access_rights('read', raise_exception=False):
    ...
    def get_records_icondata(self, **kw):
        ...
        global_search = (
            request.env["global.search.bizople"].sudo().browse(int(kw.get("id")))
        )
                    ...
                    if field.name == "display_name" or field.name == "name":
                        ...
                        try:
                            ...
                            if user.id != SUPERUSER_ID:
                                records = (
                                    request.env[model_name].sudo(False).search(domain)
                                )
                            else:
                        ...
                        field_string = (
                            request.env[str(kw.get("model"))]._fields[field.name].string
                        )
                        ...
                        if field.relation:
                            ...
                            if not request.env[field_model].check_access_rights(
                                "read", raise_exception=False
                            ):
                                continue  # Skip fields the user can't access field relation
                            else:
                                domain.append(
                                    (field.name, "ilike", kw.get("searchvals"))
                                )

                                field_string = (
                                    request.env[str(kw.get("model"))]
                                    ._fields[field.name]
                                    .string
                                )
                                domain_count = [
                                ...
                                if not request.env[model_name].check_access_rights(
                                    "read", raise_exception=False
                                ):
                                    return {
                                        "error": "You do not have access rights to view this model."
                                    }
            ...
            for rec in records:
                ...
                for field in global_search.global_field_ids:
                    if field.ttype == "many2one":
                        ...
                        try:
                            rec_field_values[field.name] = ", ".join(
                                str(value)
                                for value in rec[field.name].mapped("display_name")
                            )
                        except Exception as e:
                            # Handle the error or log it as needed
                            rec_field_values[field.name] = "Error: " + str(e)
