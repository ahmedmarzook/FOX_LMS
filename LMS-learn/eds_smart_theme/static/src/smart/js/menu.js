...
import { ColorPallet } from "@eds_smart_theme/smart/js/color_pallet";
...
patch(NavBar.prototype, {
    async setup(parent, menuData) {
        ...
        $(document).on('smart:open-theme-configurator', function () { self._openConfigModal(); });
        ...
        $(document).on('click', '.o_action_manager .top_menu_horizontal', function (ev) { self._HomeAction(ev) });
        ...
        $(document).on('click', '.o_action_manager .spiffy-submenu-group', function (ev) { self._SpiffySubMenuGroupEnt(ev) });
        ...
        $(document).on('click', '#search_bar_modal .btn-close', function(ev){self._hideSearchbarModal(ev)});
    ...
    _ToggleBookmarkPanel: function (ev) {
        ...
        jsonrpc('/update/bookmark/panel/show', {
    ...
    _all_apps_menu_data: function () {
        ...
        jsonrpc('/get/irmenu/icondata', {
            ...
            $.each(menu_data, function (key, value) {
                ...
                const horizontalMenuContainer = $(".spiffy-app-group-ent");
            
                if (!horizontalMenuContainer.data('menu-rendered')) {
                    const horizontalMenuTemplate = $(renderToElement("eds_smart_theme.AppMenuGroup", {
                    ...
                    horizontalMenuContainer.data('menu-rendered', true); // Mark as rendered
                }
                if ($('body').hasClass('top_menu_horizontal')) {
                    ...
                    if (!verticalMenuContainer.data('menu-rendered')) {
                        const verticalMenuTemplate = $(renderToElement("eds_smart_theme.AllAppsMenus", {
                            group_info: value.spiffy_app_group,
                            menu_info: menu_data,
                            getMenuItemHref: self.getMenuItemHref,
                ...
                if (current_record.use_icon) {
                    if (current_record.icon_class_name) {
                        ...
                        var icon_image = "<img class='img img-fluid' src='/eds_smart_theme/static/src/smart/images/bizople-icon.png' />"
                        $tagtarget.find('.app_icon').append($(icon_image))
                    }
                } else {
    ...
    _all_apps_records_data : function () {
        var self = this;
        jsonrpc('/get/records/global/search', {
        }).then(function (rec) {
    ...
    _SpiffySubMenuGroupEnt: function (ev) {
        $('.spiffy-submenu-group').addClass('d-none').removeClass('active');
    },
    ...
    addmultitabtags: function (ev) {
        var self = this
        jsonrpc('/get/mutli/tab', {}).then(function (rec) {
            if (rec) {
    ...
    _getModeData: function () {
        var self = this
        jsonrpc('/get/dark/mode/data').then(function (rec) {
            var dark_mode = rec
            self._ChangeThemeMode(dark_mode)
        })
    },
    addconfiguratorclass: function () {
        jsonrpc('/get/model/record').then(function (rec) {
            ...
            if (rec.record_dict[0].font_size) {
                loadCSS(`/eds_smart_theme/static/src/smart/scss/font_sizes/${rec.record_dict[0].font_size}.css`);
            }
            ...
            if (rec.record_dict[0].menu_bg_image) {
                ...
                $(".new_systray").attr("style", "background-image: url('/eds_smart_theme/static/src/smart/images/header_vertical_mini.svg')");
                $(".top_menu_horizontal.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", "background-image: url('/eds_smart_theme/static/src/smart/images/header_vertical_mini.svg')");
            }
    ...
    addbookmarktags: function() {
        jsonrpc('/get/bookmark/link', {}).then(function(rec) {
            $('.bookmark_list').empty()
            $.each(rec, function(key, value) {
                var urlParams = new URLSearchParams(value.url.split('?')[1]);
                var urlParams_actionId = new URLSearchParams(value.url.split('#')[1]);
                var menu_id = urlParams.get('menu_id');
                var actionId = urlParams_actionId.get('action');
                var Id = urlParams_actionId.get('id');
                var model = urlParams_actionId.get('model');
                var view_type = urlParams_actionId.get('view_type');
                var anchor_tag = `
    ...
    _ChangeThemeMode: function (darkmode) {
        if (darkmode) {
            jsonrpc('/active/dark/mode', { 'dark_mode': 'on' })
                .then(function (data) {
                    if (data) {
                    }
                })
            ...
            jsonrpc('/active/dark/mode', { 'dark_mode': 'off' })
                .then(function (data) {
                    if (data) {
                    }
                })
    ...
    _ChangeSidebarBehaviour: function (ev) {
        ...
        jsonrpc('/sidebar/behavior/update', {
    ...
    _GetLanguages: function () {
        ...
        jsonrpc('/get/active/lang').then(function (data) {
            var lang_list = data
            if (data && data.length > 1) {
                ...
                $.each(lang_list, function (index, value) {
                    var searchedlang = $(renderToElement("Searchedlang", {
                        ...
                        active_lang: session.user_context && session.user_context.lang
                    }))
    ...
    _showSearchbarModal: function (ev) {
        ...
        if (!this.$search_modal_popup.hasClass('show')) {
            this.$search_modal_popup.show();
            this.$search_modal_popup.addClass('show')
            this._searchModalFocus()
        } else {
            this.$search_modal_popup.hide();
            this.$search_modal_popup.removeClass('show')
            this._searchModalReset()
        }
    },

    _hideSearchbarModal: function (ev) {
        this.$search_modal_popup.hide();
        this.$search_modal_popup.removeClass('show')
        this._searchModalReset()
     },
    ...
    _searchPages: function () {
        ...
        if (searchvals_option === "menu") {
            ...
            this.$search_modal_results.empty().append(renderToFragment('eds_smart_theme.MenuSearchResults', {
                results: results,
                widget: this,
            }));
        }
            ...
            options.each(function() {
                ...
                jsonrpc('/get/records/data', {
                    ...
                    if (rec) {
                        ...
                        search_modal_results.empty().append(renderToFragment('eds_smart_theme.RecordSearchResults', {
    ...
    showToDoSidebar: function () {
        ...
        $.get('/show/user/todo/list', {}).then(function (data) {
            ...
            $(".note-save-update .note-add").on('click', function (ev) {
                ...
                jsonrpc('/create/todo', jsonDict).then(function (rec) {
                    if (is_update) {
...
export async function openThemeConfigurator(env) {
    const rec = await jsonrpc('/get/model/record');
    if (!rec.show_edit_mode) {
        env.services.notification.add(
            _t("You do not have permission to customize the theme."),
            { type: "warning" }
        );
        return;
    }
    $(document).trigger('smart:open-theme-configurator');
}

async function smartThemeConfiguratorAction(env) {
    await openThemeConfigurator(env);
}

registry.category("actions").add("eds_smart_theme_configurator", smartThemeConfiguratorAction);
