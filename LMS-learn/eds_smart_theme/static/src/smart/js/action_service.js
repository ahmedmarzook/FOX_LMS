...
import body_color from "@eds_smart_theme/smart/js/menu";
...
function makeActionManager(env) {
    ...
    function _getViewInfo(view, action, views, props = {}) {
        ...
        const viewProps = Object.assign({}, props, {
            ...
            type: view.type,
            selectRecord: async (resId, { activeIds, readonly }) => {
                if (_getView("form")) {
                    await switchView("form", { readonly, resId, resIds: activeIds });
                }
            },
            createRecord: async () => {
                if (_getView("form")) {
        ...
        if (view.type === "form") {
            if (action.target === "new") {
                // New records are editable by default (no explicit "mode" prop
                // in Odoo 19's FormController anymore — it uses a boolean
                // `readonly` prop instead), so nothing to set here.
                if (!viewProps.onSave) {
            ...
            if (action.flags && "mode" in action.flags) {
                viewProps.readonly = action.flags.mode === "readonly";
            }
        }
    ...
    async function _executeActWindowAction(action, options) {
        ...
        for (const [, type] of action.views) {
            if (type !== "search" && viewRegistry.contains(type) && session.view_info[type]) {
                const { icon, display_name, multi_record: multiRecord } = session.view_info[type];
                views.push({ ...viewRegistry.get(type), icon, display_name, multiRecord, type });
            }
        }
        if (!views.length) {
    ...
    async function doActionButton(params) {
        ...
        if (params.special) {
            ...
            const callProm = rpc("/web/dataset/call_button", {
                args,
                kwargs: { context },
                method: params.name,
