...
class OpAdmission(models.Model):
    ...
    def _check_admission_register(self):
        for rec in self:
            if not rec.register_id or not rec.application_date:
                continue

            start_date = rec.register_id.start_date
            end_date = rec.register_id.end_date
            application_date = rec.application_date.date()
    ...
    def enroll_student(self):
        for record in self:
            ...
            if not record.phone:
                raise UserError(
                    _('Please fill in the phone number.')
                )
            reg_id.get_subjects()
    ...
    def open_student(self):
        ...
        list_view = self.env.ref('openeducat_core.view_op_student_list')
        value = {
