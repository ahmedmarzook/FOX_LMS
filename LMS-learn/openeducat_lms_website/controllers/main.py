...
class OpenEduCatLms(http.Controller):
    ...
    @http.route('/openeducat_lms/fetch_course', type='jsonrpc', auth='user')
    ...
    def lms_course_create(self, *args, **kw):
        ...
        return werkzeug.utils.redirect("/course-detail/%s" % (request.env['ir.http']._slug(channel)))
    ...
    @http.route(['/lms/category/search_read'], type='jsonrpc', auth='user',
                methods=['POST'], website=True)
    ...
    @http.route(['/course/section/search_read'], type='jsonrpc', auth='user',
                methods=['POST'], website=True)
    ...
    @http.route(['/courses/add_material'], type='jsonrpc', auth='user',
                methods=['POST'], website=True)
    def create_slide(self, *args, **post):
        ...
        values = {fname: post[fname] for fname in self._get_valid_slide_post_values()
                  if post.get(fname)}
        ...
        if slide.material_type == "quiz":
            ...
            redirect_url = '/odoo/action-%s/op.material/%s' % (action_id, slide.id)

        if slide.material_type == "webpage":
            redirect_url = '/material-edit/%s?enable_editor=1' % request.env['ir.http']._slug(slide)

        return {
            'section_material_ids': section_material.ids,
            'url': redirect_url,
            'slide_id': slide.id,
            'category_id': slide.category_id.id if slide.category_id else False
        }
    ...
    @http.route(['/course/prepare_preview'], type='jsonrpc', auth='user',
                methods=['POST'], website=True)
