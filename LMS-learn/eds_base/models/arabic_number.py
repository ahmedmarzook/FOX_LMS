...
class NumberToArabic:
    ...
    def getDecimalValue(self, decimalPart):
        ...
        if self.currencyInfo.getPartPrecision() != len(decimalPart):
            ...
            for i in range(
                0, self.currencyInfo.getPartPrecision() - decimalPartLength
            ):  # @UnusedVariable
                decimalPart += "0"
                # Fix for 1 number after decimal ( 10.5 , 1442.2 , 375.4 )

            dec = (
                len(decimalPart) <= self.currencyInfo.getPartPrecision()
                and len(decimalPart)
                or self.currencyInfo.getPartPrecision()
            )
        ...
        for i in range(
            len(result), self.currencyInfo.getPartPrecision()
        ):  # @UnusedVariable
            result += "0"
    ...
    def processArabicGroup(self, groupNumber, groupLevel, remainingNumber):
        ...
        if tens > 0:
            if tens < 20:  # if we are processing under 20 numbers
                if (
                    tens == 2 and hundreds == 0 and groupLevel > 0
                ):  # // This is special case for number 2 when it comes alone in the group
                    if (
                        _intergerValue == 2000
                        or _intergerValue == 2000000
                        or _intergerValue == 2000000000
                        or _intergerValue == 2000000000000
                        or _intergerValue == 2000000000000000
                        or _intergerValue == 2000000000000000000
                    ):
                        retVal = (
                            "%s" % self.arabicAppendedTwos[groupLevel]
                        )  # // في حالة الاضافة
                    else:
                        retVal = (
                            "%s" % self.arabicTwos[groupLevel]
                        )  # //  في حالة الافراد
                else:  # // General case
                    if retVal != "":
                        # //retVal += " و ";
                    ...
                    if tens == 1 and groupLevel > 0 and hundreds == 0:
                        ...
                        if (
                            (tens == 1 or tens == 2)
                            and (groupLevel == 0 or groupLevel == -1)
                            and hundreds == 0
                            and remainingNumber == 0
                        ):
                            retVal += ""  # // Special case for 1 and 2 numbers like: ليرة سورية و ليرتان سوريتان
                        else:
                            retVal += self.getDigitFeminineStatus(
                                tens, groupLevel
    ...
    def convertToArabic(self, value, currencyCode="SAR"):
        ...
        self.numberToArabic(
            self.number,
            self.currencyInfo,
            self.englishPrefixText,
            self.englishSuffixText,
    ...
    def _convertToArabic(self):
        ...
        while tempNumber > 0:
            ...
            groupDescription = self.processArabicGroup(
                ...
                if group > 0:
                    ...
                    if numberToProcess != 2:
                        ...
                        if numberToProcess % 100 != 1:
                            ...
                            if (
                                numberToProcess >= 3 and numberToProcess <= 10
                            ):  # // for numbers between 3 and 9 we use plural name
                                retVal = "%s %s" % (
                                    self.arabicPluralGroups[group],
                                    retVal,
                                )
                                ...
                                if retVal != "":  # // use appending case
                                    ...
                                    retVal = "%s %s" % (self.arabicGroup[group], retVal)
                                    # // use normal case
        ...
        formattedNumber = ""
        formattedNumber += (
            (self.arabicPrefixText != "") and "%s " % self.arabicPrefixText or ""
        )
                    ...
                    if remaining100 == 2:
                        ...
                        if remaining100 >= 3 and remaining100 <= 10:
                            ...
                            if remaining100 >= 11 and remaining100 <= 99:
                                formattedNumber += (
                                    self.currencyInfo.arabic1199CurrencyName
                                )
            ...
            if remaining100 == 0:
                ...
                if remaining100 == 1:
                    ...
                    if remaining100 == 2:
                        ...
                        if remaining100 >= 3 and remaining100 <= 10:
                            formattedNumber += (
                                self.currencyInfo.arabic310CurrencyPartName
                            )
                        else:
                            if remaining100 >= 11 and remaining100 <= 99:
                                formattedNumber += (
                                    self.currencyInfo.arabic1199CurrencyPartName
                                )

        formattedNumber += (
            (self.arabicSuffixText != "") and " %s" % self.arabicSuffixText or ""
        )
