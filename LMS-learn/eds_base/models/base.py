...
def normilize(*names, record_id=False):
    ...
    text = re.sub(r"[\W_]", " ", name.lower()).strip().replace(" ", "_")
    res = []
    non_ascii = False
    for char in text:
        if ord(char) > 128:
...
class Base(models.AbstractModel):
    ...
    def get_dept_emails(self, dept):
        hr_emps = (
            self.env["hr.employee"]
            .sudo()
            .search([("department_id", "=", dept)])
            .mapped("work_email")
        )
        return ";".join(hr_emps)
    ...
    def _selection_name(self, field_name: str) -> str | bool:
        """
        Helper method to get the human-readable name of a selection field value.

        Args:
            field_name: The name of the selection field

        Returns:
            The human-readable name of the selection value or False if no record
        """
        if not self:
            return False

        field = self._fields[field_name]
        if not hasattr(field, "_description_selection"):
            return self[field_name]

        names = dict(field._description_selection(self.env))
    ...
    def _child_of(self, others, parent=None):
        ...
        if not (isinstance(others, BaseModel) and others._name == self._name):
            raise TypeError(
                "Comparing apples and oranges: %s._child_of(%s)" % (self, others)
            )
    ...
    def _action_view_one2many(self, field_name):
        ...
        return {
            ...
            ._get(self._name, field.name)
            .field_description,
            "res_model": field.comodel_name,
            "view_mode": ",form",
            "domain": [(field.inverse_name, "=", self.id)],
            "context": {"default_" + field.inverse_name: self.id},
        }

    def _random_password(self, length=16, chars=None):
        chars = list(
            chars or (string.ascii_letters + string.digits + string.punctuation)
        )
    ...
    def get_record_info(self):
        ...
        data = {
            ...
            "create_date": "create_date" in self
            and format_datetime(self.env, self.create_date),
            "create_uid": "create_uid" in self and self.create_uid.sudo().display_name,
            "write_date": "write_date" in self
            and format_datetime(self.env, self.write_date),
        ...
        if "audit.log" in self.env:
            ...
            data["log_count"] = (
                ...
                .sudo()
                .search_count(
                    [("record_id", "=", self.id), ("model_id", "=", model_id)]
                )
            )
            ...
            data["tracking_value_count"] = self.env["mail.tracking.value"].search_count(
                [("mail_message_id", "in", message_ids.ids)]
            )
        ...
        if log_ids:
            data["update_log_count"] = (
                self.env["audit.log.detail"]
                .sudo()
                .search_count([("log_id", "in", log_ids)])
            )
            for detail in (
                self.env["audit.log.detail"]
                .sudo()
                .search([("log_id", "in", log_ids)], order="id desc", limit=100)
            ):
                data["lines"].append(
                    {
        ...
        if message_ids:
            tracking_value_ids = self.env["mail.tracking.value"].search(
                [("mail_message_id", "in", message_ids.ids)], order="id desc", limit=100
            )
            for value_id in tracking_value_ids:
                field_info = value_id.field_info or {}
                field_type = field_info.get("type") or (value_id.field_id.ttype if value_id.field_id else "char")
                field_desc = field_info.get("desc") or (value_id.field_id.field_description if value_id.field_id else "")
                data["lines"].append(
                    {
                        "field": field_desc,
                        "old_value": value_id._format_display_value(
                            field_type, new=False
                        )[0],
                        "new_value": value_id._format_display_value(
                            field_type, new=True
                        )[0],
    ...
    def view_fields_with_data(self):
        ...
        return {
            ...
            "name": _("View Data"),
            "views": [
                (self.env.ref("eds_base.view_ir_model_fields_tree_data").id, "list")
            ],
