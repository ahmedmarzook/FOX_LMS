...
class IrModelFields(models.Model):
    ...
    default_ids = fields.One2many(
        "ir.default", "field_id", string="User-defined Defaults"
    )
    ...
    def _calc_record_value(self):
        ...
        for field in self:
            ...
            display = (
                ",".join(value.sudo().mapped(lambda r: r.display_name or repr(r)))
    ...
    def action_open_record(self):
        ...
        if len(value) <= 1:
            action.update({"views": [(False, "form")], "res_id": value.id})
