...
class LoginAs(models.TransientModel):
    ...
    group_ids = fields.Many2many(
        "res.groups", compute="_calc_group_ids", string="User Groups"
    )
    ...
    def _onchange_group_id(self):
        if self.group_id and self.group_id not in self.user_id.group_ids:
            self.user_id = False

    @api.depends("user_id")
    def _calc_group_ids(self):
        for record in self:
            record.group_ids = record.user_id.group_ids.filtered(
                lambda group: group.privilege_id and group.privilege_id.category_id.visible
            ).sorted("full_name")
