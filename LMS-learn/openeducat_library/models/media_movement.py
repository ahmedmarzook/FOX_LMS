...
class OpMediaMovement(models.Model):
    ...
    def _check_date(self):
        for record in self:
            if record.issued_date and record.return_date and \
                    record.issued_date > record.return_date:
    ...
    def check_actual_return_date(self):
        for record in self:
            if record.issued_date and record.actual_return_date and \
                    record.issued_date > record.actual_return_date:
                raise ValidationError(_(
    ...
    def create_penalty_invoice(self):
        for rec in self:
            ...
            invoice = self.env['account.move'].create({
                'partner_id': (rec.student_id.partner_id.id or rec.faculty_id.partner_id.id),
