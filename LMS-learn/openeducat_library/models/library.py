...
class OpLibraryCardType(models.Model):
    ...
    def check_details(self):
        for record in self:
            if record.allow_media < 0 or record.duration < 0.0 or \
                    record.penalty_amt_per_day < 0.0:
    ...
    def create(self, vals_list):
        ...
        for card in res:
            if card.type == 'student' and card.student_id:
                card.student_id.library_card_id = card
            elif card.type == 'faculty' and card.faculty_id:
                card.faculty_id.library_card_id = card
        return res
