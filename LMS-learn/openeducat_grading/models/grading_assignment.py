...
class GradingAssigment(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('sequence', _('New')) in ('/', _('New')):
                vals['sequence'] = (
                    self.env['ir.sequence'].next_by_code('grading.assignment')
                    or '/'
                )
    ...
    def _compute_hide(self):
        for record in self:
            record.hide = bool(record.grade and record.grade.grade_table_ids)

    @api.depends('assignment_type', 'assignment_type.assign_type')
    def _compute_assignment_types(self):
        for record in self:
            record.hide_subject = (
                record.assignment_type.assign_type == 'attendance'
            )
