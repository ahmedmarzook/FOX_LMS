...
class GradebookGradebook(models.Model):
    ...
    def open_grade_book_grid(self):
        ...
        return {
            ...
            'target': 'current',
            'params': {
                'grade_book': self.student_id.id,
                'is_gradebook': self.id,
            },
        }
    ...
    def get_grade_book_grid_data(self, student_id=None, is_gradebook=False):
        ...
        # When the Grid is opened from a specific Grade Book record, display
        # that record regardless of its workflow state.  The old code always
        # required ``state = publish`` first, so Draft/In Progress records
        # returned an empty Grid even when their gradebook JSON was available.
        if is_gradebook:
            try:
                gradebook_id = int(is_gradebook)
            except (TypeError, ValueError):
                gradebook_id = 0
            progression = self.env['gradebook.gradebook'].browse(gradebook_id).exists()
        else:
            try:
        ...
        for record in progression:
            if not record.gradebook:
                continue
            try:
                grade_data = json.loads(record.gradebook)
            except (TypeError, ValueError, json.JSONDecodeError):
                continue
            if grade_data:
                key = record.course_id.name or record.name or str(record.id)
                data[key] = grade_data
            ...
            if grade.gradebook:
                for i in grade.gradebook_line_id:
                    ...
                    if i.course_id.grade_scale_id:
                        ...
                        credit = False
                    subData[grade.student_id.name] = json.loads(grade.gradebook or '{}')
                    break
                data.append({
    ...
    def calculate_total_year(self):
        courses = self.env['op.course'].search([('id', 'in',
                                                 ...
                                                 course_id.ids)])
        if self.gradebook:
            gradebooks = json.loads(self.gradebook or '{}')
            for course in courses:
                if not course.grade_scale_id.op_grade_type_ids:
    ...
    def student_Grade_override(self):
        ...
        if self.student_id:
            terms = json.loads(self.gradebook or '{}')
    ...
    def get_json_data_in_report(self, c_id):
        ...
        if student.gradebook:
            gradebook = json.loads(student.gradebook or '{}')
