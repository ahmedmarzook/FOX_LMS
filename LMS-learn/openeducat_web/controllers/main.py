...
class OpenEduCatWebController(http.Controller):
    ...
    @http.route('/db_register', type='http', auth='user', website=True, csrf=True)
    def db_register(self, **post):
        ...
        if post and post.get('instance_key'):
            ...
            if not request.env['res.config.settings'].sudo().request_verify_instance_controller(
        ...
        if post and post.get('instance_hash_key'):
            ...
            if request.env['res.config.settings'].sudo().request_verify_hash(
