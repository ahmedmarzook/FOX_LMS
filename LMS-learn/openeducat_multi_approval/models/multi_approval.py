...
class MultiApproval(models.Model):
    ...
    def _compute_approval_minimum(self):
        for approval_type in self:
            approval_type.approval_minimum = len(
                approval_type.line_ids.filtered(lambda line: line.require_opt == 'required')
            )

    def _compute_to_review_number(self):
        for rec in self:
            rec.request_to_validate_count = self.env['multi.request'].search_count(
                [('approval_type_id', '=', rec.id),
                 ('line_ids.user_id', '=', self.env.uid),
    ...
    def onchange_model_id(self):
        # Never store the literal string 'False': the domain widget would then
        # try to call a non-existent model named False.
        self.model_name = self.model_id.model or False
    ...
    def action_configure(self, view_id=None, view_type='form',
        ...
        self.env['ir.model.fields'].search([('name', '=', 'x_is_hide'),
                                            ('model_id', '=', self.model_id.id)])
        b = "not x_is_request or x_is_view_approval"
        va = "not x_is_view_approval"
        na = "not x_is_request or x_result_approval"
        ha = "x_result_approval != 'approve' or not x_is_request"
        ra = "x_result_approval != 'refuse' or not x_is_request"
